...
function SaleContent() {
  ...
  // Toàn bộ 49 sản phẩm ưu đãi trực tiếp, không phân chia theo chiến dịch phụ
  const allProducts = SALE_PRODUCTS_DATA;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 space-y-6 sm:space-y-8">
          ...
          <div className="order-1 md:order-1 md:col-span-7 lg:col-span-7 space-y-3.5 sm:space-y-4 text-center md:text-left">
            ...
            <div className="pt-1 flex flex-wrap items-center justify-center md:justify-start gap-3">
              <button
                type="button"
                onClick={() => {
                  document.getElementById('all-sale-products')?.scrollIntoView({ behavior: 'smooth' });
                }}
      ...
      {/* ==================== TIÊU ĐỀ: TẤT CẢ SẢN PHẨM ƯU ĐÃI ==================== */}
      <section id="all-sale-products" className="pt-2 pb-2 border-b border-cream-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <h2 className="text-lg sm:text-2xl font-extrabold font-heading text-charcoal-900 tracking-tight">
              Tất Cả Sản Phẩm Ưu Đãi
            </h2>
            <span className="text-xs sm:text-sm font-bold px-2.5 py-0.5 rounded-full bg-rose-100 text-rose-600">
              {allProducts.length} sản phẩm
            </span>
          </div>
          <span className="text-xs text-charcoal-500 hidden sm:inline font-medium">
            ✨ Tuyển chọn toàn bộ thiết kế thêu tay thủ công cao cấp cho bé yêu
          </span>
        </div>
      </section>
