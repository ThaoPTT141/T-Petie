...
export default function BeGaiSetDoPage() {
  ...
  const filteredProducts = useMemo(() => {
    let list = allProducts.filter((p) => p.category === 'be-gai' && p.subcategory === 'set-do');
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
      <Breadcrumb
        items={[
          ...
          { label: 'Set Bộ Phối Sẵn', href: '/be-gai/set-do' },
        ]}
      />

      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-extrabold font-heading text-charcoal-900 mb-2">
          Set Bộ Phối Sẵn Bé Gái ✨
        </h1>
        <p className="text-xs sm:text-sm text-charcoal-600 max-w-2xl leading-relaxed">
          Tiết kiệm thời gian phối đồ cho mẹ với những set bộ áo kèm chân váy/quần bloomer được mix-match chuẩn gu Hàn Quốc.
        </p>
      </div>

      <FilterBar
        activeSubcategory="set-do"
        sortBy={sortBy}
