...
export default function BeGaiAoPage() {
  const allProducts = localProducts as Product[];
  const [sortBy, setSortBy] = useState('newest');
  const [priceRange, setPriceRange] = useState('all');

  const filteredProducts = useMemo(() => {
    let list = allProducts.filter((p) => p.category === 'be-gai' && p.subcategory === 'ao');
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
      <Breadcrumb
        items={[
          ...
          { label: 'Áo Sơ Mi & Áo Kiểu', href: '/be-gai/ao' },
        ]}
      />

      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-extrabold font-heading text-charcoal-900 mb-2">
          Áo Sơ Mi &amp; Áo Kiểu Bé Gái 👚
        </h1>
        <p className="text-xs sm:text-sm text-charcoal-600 max-w-2xl leading-relaxed">
          Thiết kế cổ sen thêu hoa, tay phồng babydoll xinh xắn từ chất vải cotton tự nhiên thoáng mát.
        </p>
      </div>

      <FilterBar
        activeSubcategory="ao"
        sortBy={sortBy}
